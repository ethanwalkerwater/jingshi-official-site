...
如果运营明确要求临时展示分，必须同时添加 `ratingSource: "temporary"`，并在 `docs/教师评分数据.md` 记录。该分数只存在官网教师主数据中，不得写入月度、累计或 accumulated 文件。下次评分更新必须先搜索 `ratingSource: "temporary"`；若该老师已有真实有效分数，用累计值替换临时分并删除此标记，临时分绝不参与计算。
